...
if graf
    ...
    inicio_fim = 24*round((gait + gait_c/2)/taxa);
    meio = 24*round(2*(gait + gait_c)/taxa);
    
    if csv
        fileID = fopen('incicio.bin','w');
        fwrite(fileID,saida(1:inicio_fim),'int16','l');
        fclose(fileID);
        
        fileID = fopen('passo.bin','w');
        fwrite(fileID,saida(inicio_fim:inicio_fim + meio),'int16','l');
        fclose(fileID);
        
        fileID = fopen('fim.bin','w');
        fwrite(fileID,saida(length(saida) - inicio_fim:length(saida)),'int16','l');
        fclose(fileID);
    end    
end
