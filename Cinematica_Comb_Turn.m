...
for cont_passos=1:passos
    for j=1:round(gait/taxa) 
        if cont_passos == 1
            ...
            Qc(round(gait/taxa)*(cont_passos-1)+j,1)=ang_q*sin(0.5*pi*taxa*j/gait*frac);
            Qc(round(gait/taxa)*(cont_passos-1)+j,2)=ang_q*sin(0.5*pi*taxa*j/gait*frac);
            Qc(round(gait/taxa)*(cont_passos-1)+j,3)=ang_q*sin(0.5*pi*taxa*j/gait*frac);
            Qc(round(gait/taxa)*(cont_passos-1)+j,4)=ang_q*sin(0.5*pi*taxa*j/gait*frac);
            else
            Qc(round(gait/taxa)*(cont_passos-1)+j,1)=Qc(round(gait/taxa)*(cont_passos-1)+j-1,1);
            ...
            Qc(round(gait/taxa)*(cont_passos-1)+j,4)=Qc(round(gait/taxa)*(cont_passos-1)+j-1,4); 
            end
        else
            if j < round(gait/taxa/frac)
            Qc(round(gait/taxa)*(cont_passos-1)+j,1)=ang_q*cos(pi*taxa*j/gait*frac1)*(-1)^(mod(cont_passos,2));
            Qc(round(gait/taxa)*(cont_passos-1)+j,2)=ang_q*cos(pi*taxa*j/gait*frac1)*(-1)^(mod(cont_passos,2));
            Qc(round(gait/taxa)*(cont_passos-1)+j,3)=ang_q*cos(pi*taxa*j/gait*frac1)*(-1)^(mod(cont_passos,2));
            Qc(round(gait/taxa)*(cont_passos-1)+j,4)=ang_q*cos(pi*taxa*j/gait*frac1)*(-1)^(mod(cont_passos,2)); 
            else
            Qc(round(gait/taxa)*(cont_passos-1)+j,1)=Qc(round(gait/taxa)*(cont_passos-1)+j-1,1);
            Qc(round(gait/taxa)*(cont_passos-1)+j,2)=Qc(round(gait/taxa)*(cont_passos-1)+j-1,2); 
            Qc(round(gait/taxa)*(cont_passos-1)+j,3)=Qc(round(gait/taxa)*(cont_passos-1)+j-1,3); 
...
for cont_passos=1:passos
    for j=1:round(gait/taxa)
        ...
        switch escolha
            case 1
            Q_tr(round(gait/taxa)*(cont_passos-1)+j+1,1)=ang_tr*sin(0.5*pi*taxa*j/gait);
            Q_tr(round(gait/taxa)*(cont_passos-1)+j+1,2)=0; %ang_tr*sin(0.5*pi*taxa*j/gait)*(-1)^(1-mod(cont_passos,2));
            case 0
            Q_tr(round(gait/taxa)*(cont_passos-1)+j+1,1)=ang_tr - ang_tr*sin(0.5*pi*taxa*j/gait);
            Q_tr(round(gait/taxa)*(cont_passos-1)+j+1,2)=0; %ang_tr*sin(0.5*pi*taxa*j/gait)*(-1)^(1-mod(cont_passos,2));
        end
     end
end
